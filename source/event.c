...
{
    gpe_block_apply_action(block, GPE_BLOCK_ACTION_DISABLE_ALL);
...
static uacpi_bool gpe_masked(struct gp_event *event)
{
    return event->reg->masked_mask & gpe_get_mask(event);
}
...
)
{
    ...
    was_masked = gpe_mask_safe(event);
