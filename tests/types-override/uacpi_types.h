...
/*
 * A copy of the default platform types header with NULL explicitly
 * undefined at the end. Used by CI to ensure the library never refers
 * to NULL directly instead of UACPI_NULL by compiling every source
 * file with this header (via UACPI_OVERRIDE_TYPES), making any such
 * reference a hard error.
 */
